...
public final class PlaceCategoryKeywords {
  ...
  private static Map<String, List<String>> buildOrderedRestaurantKeywords() {
    ...
    // "주점"(2026-08-31 추가) - 소상공인시장진흥공단 API 업종명("주점 요리 주점", "주점 일반 유흥
    // 주점") 매칭 실패 샘플에서 발견. 이자카야/포차/호프/와인바보다 뒤에 둬서 더 구체적인 키워드가
    // 먼저 걸리게 한다.
    m.put("술집/포차/이자카야", List.of("이자카야", "포차", "호프", "와인바", "주점"));
    ...
    // 베이커리/제과점(2026-08-31 신규) - 소상공인 매칭 실패 샘플에서 압도적 1위(64건 중 22건)로
    // 나온 "기타 간이 빵/도넛 제과점업" 패턴. "베이커리"는 바로 위 "베이커리카페"와 겹치니 빼고,
    // 실측 텍스트에 있는 "제과점"/"빵집"만 키워드로 둔다.
    m.put("베이커리/제과점", List.of("제과점", "빵집"));
    ...
    // "베트남"(2026-08-31 추가) - 기존 "베트남음식"이 소상공인 업종명 실측 텍스트("동남아시아
    // 베트남식 전문 기타 외국식 음식점업")의 "베트남식"과 안 맞아떨어지던 문제 수정.
    m.put("식당(아시아음식)", List.of("베트남", "태국음식", "쌀국수", "팟타이", "인도음식", "커리"));

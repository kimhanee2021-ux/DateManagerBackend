...
public class Couple {
  ...
  // Couple row를 재사용하는 정책으로 바뀌면서 더 이상 맞지 않는 전제가 됐다(CoupleServiceImpl.
  // acceptInvite() 참고). 재결합할 때마다 이 값을 지금 시각으로 갱신해야 "이 사이트에서 연결된
  // 지 며칠째" 표시가 예전 연결 기간까지 포함해서 부풀려지지 않는다. 관리자 페이지의 "다시 연결"
  // 버튼(AdminCoupleServiceImpl)도 같은 이유로 이 값을 갱신한다.
  @Setter
  @Builder.Default
  ...
  // CoupleServiceImpl.acceptInvite()에서는 이 값을 건드리지 않고 null로 둔다.
  @Setter
